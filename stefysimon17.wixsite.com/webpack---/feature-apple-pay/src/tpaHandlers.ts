...
export const ApplePayTPAHandlers = withDependencies([ViewerModelSym], ({ siteAssets }: ViewerModel) => ({
	getTpaHandlers() {
		...
		const applePayStartSession = async (
			...
			try {
				...
				const pmRpc = (await loadPmRpc(siteAssets.clientTopology.moduleRepoUrl)) as PmRpc
				...
				session.onshippingmethodselected = async (e: ApplePayJS.ApplePayShippingMethodSelectedEvent) => {
					...
					return session.completePaymentMethodSelection(update)
				}
