...
function sanitizeEmail(email) {
    ...
    return hashers_1.hashEmail(email);
}
...
function sanitizePhone(phone) {
    if (!validators_1.validatePhone(phone)) {
        return phone;
    }
    return hashers_1.hashPhone(phone);
}
...
function sanitizePII(text) {
    if (!validators_1.validateString(text)) {
        return text;
    }
    return replacers_1.replaceEmails(text, function(email) {
        return hashers_1.hashEmail(email);
    });
}
...
function hashString(str) {
    if (!validators_1.validateString(str)) {
        return str;
    }
    return hashers_1.digest(str);
}
